...
In this section we assume the Online Balanced Partition problem with the existence of a perfect partition of the input graph.

\begin{theorem}
  The competitive ratio of any deterministic algorithm for Online Balanced Partition (with perfect partition) is $\Omega(k\cdot \ell)$.
\end{theorem}
...
\begin{proof}
  ...
  Fix a cluster $A$ containing nodes $a_1, \ldots, a_k$ and a cluster $B$ containing (among others) a node $b$.
  ...
  \maciek{Possibly we need a better name than $x$. Something that indicates that this changes over time, possibly tied to the name DET.}
  We define $x$ to be the node in the configuration of DET that is co-located in one cluster with the component $a_1, \ldots, a_{k-1}$.
  Initially, $x = a_k$.
  
  Then, the adversary issues an external request $(x, b)$, which at this point is the request $(a_k, b)$.
  Note that DET must relocate the component $\{a_k, b\}$ to some other cluster.
  OPT also relocates the component $\{a_k, b\}$ to some carefuly chosen cluster $D$ (to be fixed later), and it migrates nodes $d_1$ and $d_2$ to the previous locations $a_k$ and $b$ (i.e., to the clusters $A$ and $B$).
