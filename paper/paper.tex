...
%and every algorithm must colocate every communicating pair.
...
follows. There is a set of $n$ nodes,
initially arbitrarily partitioned into $\ell$~clusters,
each of size~$k$.
Nodes in~$C \subset V$ are \emph{collocated} by the partition $P$
if $P$ assigns them to the same cluster.
I.e.,
$\forall u,v \in C: \mathit{cluster} (u,P) = \mathit{cluster} (v,P)$.
The input to the problem is a sequence of communication requests
$\sigma = (u_1,v_1),$ $(u_2,v_2),$ $(u_3,v_3), \ldots$,
where a pair $(u_t,v_t)$ indicates that nodes $u_t,v_t$ exchange a fixed amount of data.
An online algorithm needs to serve the communication request~$(u_t,v_t)$
as soon as it arrives.
Before serving the request,
the online algorithm
can re-partition,
i.e.,
it may move some of the nodes into clusters different than their current assigned clusters.
We assume that
a~communication request between two collocated nodes costs 0,
the cost of a~communication request between two nodes located in different clusters is~1,
and the cost of migrating a node from one cluster to another
is~$\alpha \in \mathbb{Z}^+$.
For any algorithm $ALG$,
its cost,
denoted by $ALG(\sigma)$,
is the total cost of communications and
the cost of migrations on the sequence $\sigma$.
...
using an input sequence that allows OPT to move to a perfect partition.
Therefore,
it is a lower bound for OBRP with perfect partition,
which directly implies a lower bound for the (general) OBRP.
The former complements an upper bound in $O(k \cdot \ell)$
for the OBRP with perfect partition which we present in Section \ref{sec:upperbound}.
...
Let ALG be any online algorithm for OBRP.
...
Recall that internal requests does not incur ALG any cost.
If ALG at any point splits a component then
we issue  requests between the parts as many as it is necessary for ALG to collocate the parts,
as otherwise ALG is not competitive.
...
\begin{lemma} \label{lemma:k=3}
  The re-balancing cost of GREEDY for $k=3$ is $O(1)$.
