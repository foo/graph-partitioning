...
\maciek{Here, we would like to introduce insights about the proof, emphasize the conditions for the bound to work (such as $k\geq 3$), and maybe compare to previously known lower bounds. Also, introduce the notation and definitions that are needed, such as the concepts of components (that we force alg to keep on the same server).}
...
\TAlg partition nodes into components.
...
\begin{proof}
  ...
  The subsequence $\sigma_E$ contains requests to pairs that belong to the same component of \TAlg{} at the end of the phase, and $\sigma_E$ denotes the requests to pairs that did not belong to the same component of \TAlg.
