...
\TAlg maintains the components for each node.
...
If no such partition exists, \TAlg resets all its components and finishes the phase.

\begin{lemma}
  Every repartition of \TAlg has the cost $O(1)$.
\end{lemma}

\begin{theorem}
  In a single phase, \TAlg pays $O(\ell)$.
\end{theorem}
...
\end{theorem}
\begin{proof}
  OPT pays at least $1$ in a single phase.
\end{proof}
