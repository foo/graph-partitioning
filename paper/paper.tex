...
\begin{proof}
  ...
  Moreover, \TAlg paid for all requests from $\sigma_E$, and we bound it $\TAlg(\sigma_E) = |\sigma_E|$.
  ...
  In a fixed configuration of $\OPT$, it might mitigate paying for requests between at most $3\ell$ pairs of nodes by collocating them in its clusters.
  Recall that $\sigma_E$ consists of requests between unsaturated pairs, and it accounts only for requests that increased the counter (i.e., external requests), thus $\OPT$ might mitigate at most $3\ell\cdot(\alpha - 1)$ requests from $\sigma_E$.
  ...
  This might allow to serve requests between $(u,u')$, $(u,u'')$, $(v,v')$ and $(v,v'')$ for free afterwards.
  As $\sigma_E$ consists of requests between unsaturated pairs, and it accounts only for external requests, there are at most $\alpha-1$ requests between each of these pairs.
  Thus, by performing a single migration for the cost $\alpha$, $\OPT$ may not pay the remote serving costs for at most $4\cdot (\alpha - 1)$ requests from $\sigma_E$.
  Finally, for serving $\sigma_E$, $\OPT$ pays at least $W \cdot \frac{\alpha}{4\cdot (\alpha-1)}$.
  This gives us
  \begin{equation*}
    \OPT(\sigma_E) + \OPT(M)  \geq (|\sigma_E|-3\cdot\ell\cdot(\alpha - 1)) \cdot \frac{\alpha}{4\cdot (\alpha-1)}
    \geq |\sigma_E| / 4 - 4\cdot \ell \cdot \alpha.
  \end{equation*}
  From that, we obtain $|\sigma_E| \geq 4\cdot(\OPT(\sigma_E)+\OPT(M)) + 16\ell \cdot \alpha$.
