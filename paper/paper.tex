...
\begin{proof}
  ...
  We obtain $\OPT \geq \max\{\alpha, \OPT(\sigma_E) + \OPT(M)}$.

  \medskip

  Before we bound the competitive ratio, we relate the costs of $\TAlg$ and $\OPT$ related to $\sigma_E$.

  Second, we estimate the cost related to $\sigma_E$.
  Now we bound the cost that $\OPT$ incurred while serving requests $\sigma_E$.
  If $\OPT$ would 
  
  \medskip

  Now we bound the competitive ratio.
