...
\begin{enumerate}
  ...
  \begin{table*}
    ...
    \renewcommand{\arraystretch}{1.5}
      ...
      \textbf{Clusters/capacity} & \textbf{ Lower bound} &\textbf{Upper bound}\\ \hline 
        \textbf{$k=2$}& 3~\cite{repartition-disc} & 3~(\S 9.3) \\ 
        \rowcolor{gray!25}
        \textbf{$k=3$}&  $1357\cdot \ell - 245 = \Omega(\ell)$ (\S 9.3)& $O(k \cdot \ell)$~\cite{repartition-disc}\\
        \textbf{$\ell=2$}&  $2k-1$ (\S 9.3)& $O(k^2)$~\cite{repartition-disc}\\
        \rowcolor{gray!25}
        general $k$ and $\ell$& $\Omega(k\cdot \ell)$ (\S 9.3)&$O(k^2 \cdot \ell^2)$~\cite{repartition-disc} \\
      \end{tabular}
  ...
  Table with summary of all results. Tight for k=2, asymptotically tight for k=3, and a difference in general lk.

  No bypassing model: reserach.
