...
We say that the request $(u,v)$ is \emph{external}, if the algorithm that receives this request is in the configuration, where $u$ and $v$ are located at different servers.
...
Upon receiving an external request to a pair, it increases the counter by $1$.
...
\begin{proof}
  ...
  Combining those bounds gives us $\OPT \geq \max\{\OPT(\sigma_I) + \OPT(M), \OPT(\sigma_E) + \OPT(M)\} \geq (\OPT(\sigma_I) + \OPT(M) + \OPT(\sigma_E) + \OPT(M)) / 2$.
  ...
  If we have at least two phases, then we split the cost $\alpha$ charged in the penultimate phase into last two phases, and follow the analysis regarding the unsaturated requests
