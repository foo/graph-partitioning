...
\begin{proof}
  ...
  By performing a single migration that costs $\alpha$, $\OPT$ may avoid paying the remote serving costs for at most $4\cdot (\alpha - 1)$ requests from $\sigma_E$.
  Thus, for serving $\sigma_E$, $\OPT$ pays at least $W \cdot \frac{\alpha}{4\cdot (\alpha-1)}$.
  This gives us
  \begin{equation*}
