...
In this paper, we use the techinique of maintaining \emph{components} of nodes, similar to previous approaches~\cite{repartition-disc}. A~\emph{component} is a subset of frequently communicating nodes.
Initially,
...
The adversary constructs components of nodes, and $\OPT$ never splits them.
The adversary observes the actions of an online algorithm, and if it splits the components at any moment, the adversary issues 
...
\todo{ACM format for review? Is there any specific format like this? Author anonymous, with line numbers etc.}
...
\todo{Consistency: collocated at OR collocated in OR collocated on}
