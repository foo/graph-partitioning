...
\begin{proof}
  ...
  Recall that $\sigma_E$ consist of requests between unsaturated pairs, and it accounts only for requests that increased the counter (i.e., external requests), thus $\OPT$ might mitigate at most $3\ell\cdot(\alpha - 1)$ requests from $\sigma_E$.
  Faced with 

  We begin by lower-bounding the number of requests from $\sigma_E$ that would incur a cost for $\OPT$ if it .
  
  may  account (by either serving remotely or migrating), and we denote their number by $x$.
