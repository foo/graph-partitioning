...
\maciek{ every algorithm must colocate every communicating pair.
	\mahmoud{updated!}}
We  focus on a restricted variant of   \PPOBRP{}
...
\todo{Font for OPT, ALG, DET, dist etc}
  ...
  Upon receiving a request $(u,v)$, \TAlg merges the components of $u$ and $v$, and moves to the closest partition, where it each component has all of its nodes in one server.

  \maciek{Component reseting magic}
  Now we define what \TAlg does when no such partition exists.
  If the resulting component (of $u$ and $v$) has size $4$, then \TAlg resets all internal requests that concern this component.
  Otherwise, \TAlg resets all its components and ends the phase.
  
  \maciek{Define "component-respecting" algorithm}
