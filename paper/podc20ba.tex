...
\begin{proof}
	Let $\alpha=1$. We construct an instance of the problem with $\ell$ clusters 
...
\begin{proof}
  ...
  Then, we cannot charge $\OPT$ due to the unability to partition the components.
...
\todo{Author anonymous conference! Done}

\todo{ACM format for review? Is there any specific format like this? With line numbers etc. Done}


\todo{CCS concepts are mandatory}

\todo{ACM Reference format starts with a "." Possibly we did not provide all info. Done}
