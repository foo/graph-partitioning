...
\begin{theorem}
  A single repartition of DET2 migrates nodes among at most $p(k)^2+(k+1)$ clusters, where $p(k)$ is the number of integer partitions $k$.
  The corollary is, the cost of the repartition is bounded by $O(k \cdot \alpha \cdot e^{c\cdot \sqrt{k}})$ for a constant $c$.
\end{theorem}

\begin{proof}
  \maciek{Danger zone, nothing is finished here}

  A \emph{signature} of a cluster $C$, denoted $S(C)$ is a multiset of sizes of components located in this cluster.
  Note that there are $p(k)$ possible signatures.

  Fix a repartition.
  
  Consider the repartition from the perspective of a single cluster.
  Prior to the repartition, it had a signature $S^-(C)$, and after the repartition it has a signature $S^+(C)$.

  Now we construct a graph $G$ of component exchange among clusters.
  The vertices are the clusters.
  If a pair of clusters $C_1, C_2$ exchange components during the repartition, we produce an edge and we label it with the \emph{signature of exchange}, i.e., a pair of cluster signatures (both ways), limited to the exchanged clusters between $C_1,C_2$ only (i.e., not necessarily summing to $k$, and  note that also not necessarily having the same size!).
  
  Proof by contradiction.
  Assume that there exists a component in $G$ of size greater than $p(k)^2+(k+1)$.
