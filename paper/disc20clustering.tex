...
In Section \ref{ssec:cascade}, we show that among all migration graphs for given component merge, there exists an optimal graph with the bounded path length.
...
In this section we present an example of a merge action that results in a $k/3$(?) length path in an optimal migration graph.
...
We begin, by bounding the length of each path in one of optimal solutions.
...
\begin{theorem}
  ...
  There exists an optimal cost reconfiguration with the migration graph, where each path have length at most $k$.
\end{theorem}

\begin{proof}
  Assume the contrary, i.e., for each optimal cluster reconfiguration, its migration graph contains a path at least $k+1$.

Fix a reconfiguration and its migration graph that has the shortest length $L$ of the longest path, and has the least number of paths of length $L$.
Let $p = \lbrace v_1, v_2, \ldots, v_L \rbrace$ be any path of length $L$.
