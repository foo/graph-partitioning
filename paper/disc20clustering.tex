...
\begin{proof}
	...
	From the bound on \OPT's cost we have $|\sigma_E| \leq 2(\OPT(\sigma_E)+\OPT(M)) + 4\alpha \cdot \ell$.
...
On the practical side, it would also be interesting to study our algorithms empirically, under realistic workloads.h
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\STATE {For each node $v$ create a component $C_v = \{ v \}$ and add it to $\mathcal{C}$.}
		%\STATE{$P := P_I$}
