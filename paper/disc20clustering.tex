 ...
 \usepackage[font={footnotesize}]{caption}https://www.wykop.pl/
...
%	Throughout the construction, we maintain ground sets for each node.
	...
	At any time,
	we refer to the isolated node currently collocated with $B$ as the \emph{special} node.
	Let $x_0$ denote the first special node.

	Then, we join the special node to a~larger ground set to force its eviction.
	Precisely,
	we create a~ground set $\{x_0, y_0\}$, 	where
%	 $x_0$ is the current special node and
...
%	denoted by $x_1$.
	...
	We proceed by joining the current special node to a~larger ground set in a similar way,
	...
	Otherwise,
	no such feasible partition exists,
	we terminate the process and
	 we let $x^* := x_i, y^*:=y_i$. 
