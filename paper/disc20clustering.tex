...
\begin{proof}
	...
	First, where we form a ground set of size $k-1$ on an arbitrarily chosen cluster $S$.
	%We never dismiss any constructed ground set.
	Each cluster host exactly $k$ nodes, hence in any configuration of \ALG, there exists a single alone node in $S$.

	Then, we construct a ground set containing this alone node (and some other node).
	Precisely,
	...
	\ALG has them split, and the adversary produces external requests until these are collocated.
	$S$ already contains a ground set of size $k-1$ (that it cannot split), hence \ALG must collocate $\{x_0, y_0\}$ in another cluster.
	

	We continue to construct ground sets containing the node that is currently alone in $S$.
	...
	Consequently, we merge the ground sets of $x_1$ and the largest ground set $C$ s.t. $C \neq \{x_0,y_0\}$ and $I(C) = I(x_1)$.
	Repeating this, we observe a sequence $X := x_0, x_1, x_2\dots$ of alone nodes that visit $S$.
	...
	We claim that constructing such ground sets can continue as long as at least $\ell$ alone node exists.
	...
	Otherwise, $\{x_0, y_0\}$ is present on $I(x_i)$ (a single ground set cannot occupy $I(x_i)$, as without $x_i$ its size is at most $k-1$).
	...
	To keep \OPT's cost low, we end constructing ground sets prematurely --- while there are still $\ell+1$ alone nodes (i.e., not performing two last constructions).
	Then, there two alone nodes exists that originate from the same cluster (we have $\ell+1$ alone nodes and $\ell$ clusters).
	...
	\ALG performs at least one swap for each ground set $\langle \{x_0, y_0\}, \{x_1, y_1\}, \{x_2, y_2\}, \ldots \rangle$.
	We start with $k \cdot \ell - (k-1)$ single nodes (a single $k-1$ ground set is revealed prior to $\{x_0, y_0\}$), and we end with $\ell+1$ nodes.
