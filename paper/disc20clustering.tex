...
We say that the node that belongs to trivial component or a ground set is single.
...
To show it, clusters of capacity $k\geq 3$ are sufficient.
...
\begin{proof}
	...
	\maciek{The problem with the definition! The first pair does not have the same initial location of its nodes.}
	Initially all nodes are single (they belong to their own trivial ground sets).

	In our construction we distinguish a cluster $S_1$, where we form a ground set of size $k-1$.
	Let $x_0$  be the only single node left on $S_1$ and let $y_0$ be an  arbitrary node that does not reside in $S_1$.
	First,
	...
	To preserve a feasible partition of nodes into clusters,
	 replaces $x_0$ and $y_0$ with two other single nodes $x_1$ and $y_1$ respectively.
	 \maciek{Not covering all cases. $y_0$ may stay in its place. This sentence should be about $x_0$ replaced by $x_1$ only.}
