...
%	The remaining cost will be used in a latter case.
  ...
  \item Case iii)
	$t'' \geq 0$ and OPT has $\set{u,v}$ collocated before $t''$ and keeps them collocated during $[t'',t]$.
	In this case OPT doesn't pays any cost for requests in $\sigma_{\set{u,v}}[t'',t]$ and
	we can't charge it directly.
	However,
	OPT must pay $\lambda$ for all requests in $\sigma_{\set{u,v'}}[t'',t']$,
	 i.e.,
	 Case (i) applies to $\set{u,v'}$  at $t'$ and
 	we charge only $\lambda/2$ to OPT (at $t'$).
	This means,
	we can charge OPT an additional $\alpha$,
	i.e.~the remaining cost of requests $\sigma_{\set{u,v'}}[t'',t']$.
...
Missing:
\maciek{Reason that the cost sharing between cases C1 and C3 is possible. Unclear to me, why every time C3 happens there must be a corresponding C1. \mahmoud{done}}
...
\maciek{"Thus, OPT pays ... which gives the competitive ratio" It is too early to give the competitive ratio if in the next paragraph we consider yet another case. \mahmoud{done!}}
