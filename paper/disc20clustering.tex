...
\maciek{Needed? "An~algorithm is \emph{component respecting}
...
We say that a component or a ground set is \emph{trivial}, if it contains a single node.
...
This way, the optimal algorithm may move to the ground-set-respecting configuration, and serve all requests for free.
(This implies that the algorithm must eventually move to a ground-set-respecting configuration if it is competitive.)
