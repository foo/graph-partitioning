...
Cost of cluster reconfiguration is the number of exchanged nodes, and from the standpoind of a migration graph, it is the sum of labels on edges of the graph.
It is bounded by $2k$ times the size of the connected component.

Core of the graph and the component merge.

Multiple migration graphs for a component merge exist, and multiple of them might have the optimal cost.
...
\subsection{Bounds on the size of a cluster reconfiguration graph}
\label{ssec:cascade}
...
\begin{proof}
  Assume that there exists a cluster reconfiguration
