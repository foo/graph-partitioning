...
Then, in any configuration, there exists an alone node on $S$.
In such setting, we only issue requests between the current alone node from $S$, and some node that was initially collocated with it.
...
\begin{proof}
	...
	Fix the moment just before the creation of a ground set with the node $x_i$.\maciek{no $x_i$ def any more}
...
The algorithm for $k=3$ from Section~\ref{sec:k3} can be efficiently distributed.
There are two non-local operations:
\begin{enumerate}
	\item We broadcast the end of the phase to reset all counters.
	\item Merge of two  may require finding a third cluster that contains $3$ alone nodes (For the details, see the proof of Lemma~\ref{lem:1req})).
\end{enumerate}

If a component grows to size $4$ or fresh cluster is found, we conclude that no perfect partition exists, and we broadcast (\maciek{TODO: efficient broadcast cite}) the message to end the phase and reset the counters in all clusters.

Let's call the cluster containing $3$ alone nodes \emph{fresh}.
Now we show how to efficiently find a fresh cluster.
We organize the clusters into an arbitrary balanced binary tree (\maciek{TODO: how to do it efficiently? Does not matter much, as this is performed once at the beginning}), and broadcast the root.
Each cluster counts the number of fresh clusters in its subtree.
...
The operation (ref1) requires a single broadcast, and the operation (ref2) requires $O(\log(\ell))$ communication.
Each of these may occur on every request.

\maciek{Possible improvement: we can have a fixed number of rounds for finding the X cluster by maintaining a direct pointer to an X cluster (not only to subgroup). However, the fix still requires the log number of rounds. But we improved the responsiveness.}
