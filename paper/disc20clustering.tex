...
%	we terminate the process.
	...
	is less than $\ell+3$.
	At each step $i$,
	the number of isolated nodes decrease either by one,
...
\PPL creates a new components by merging
...
\begin{minipage}[b]{.55\textwidth}
	...
	\useasboundingbox  (-3mm,-5mm) rectangle (0mm,10mm);
	%cluster 1
	\def\x{30}
	\def\w{15}
	%Shape: Rectangle
	...
	\draw[dotted, shorten <= -6pt, shorten >= -6pt] (a4)  -- (a1);
	...
	\draw[dotted, shorten <= -4pt, shorten >= -3pt] (b2)  -- (b3);
	...
	\draw[dotted, shorten <= -4pt, shorten >= -3pt] (a2)  -- (a3);
	\draw [->,blue, shorten <= -5pt, shorten >= -2pt] (a2) -- (b4);
	
	\draw[dotted,  shorten <= -3pt, shorten >= -5pt]  (b4) to[out=-100,in=-80] (b1);
	% arrow
...
\begin{minipage}[b]{.45\textwidth}
	\begin{tikzpicture}[x=0.8pt,y=0.75pt,yscale=-1.25,xscale=1.25]
	\useasboundingbox  (0mm,-5mm) rectangle (0mm,10mm);
%cluster 1
\def\x{30}
\def\w{15}
%Shape: Rectangle
...
\end{minipage}
	\caption{
		...
		An arrow from node $x$ to node $y$ indicates that $x$  replaces $y$.
		\OPT collocates 4 pairs from the left partition, with  4 migrations,
		 resulting in the right partition.
	}
	\label{fig:cyclic}
\end{figure}
...
\stefan{I think we can delete this paragraph: now we discussed everything
in the individual sections, no?}
...
\begin{comment}
\begin{figure}[H]	\label{fig:timeline}
	\centering
\begin{tikzpicture}
\draw [->] (-10mm,0) -- (5,0);
...
\end{tikzpicture}
\caption{
Illustration of the timeline used in the proof of Theorem \ref{thm:k=2}.
Requests in $R_t$  are only to  $\{u,w\}$ and $\{u,v\}$,
which  arrive  during the interval $(t'',t]$.
Similarly,
request in $R_{t'}$  are to $\{u,w\}$ and some other pair, irrelevant to the analysis.
Hence, requests to $\{u,w\}$ are included in at most two such sets,
which are $R_{t'}$ and $R_t$.
This is because their intervals overlap on $(t'',t']$, shown with red thick line.
}
\end{figure}
\end{comment}
