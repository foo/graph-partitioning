...
In any configuration, there must exist an isolated node collocated with the ground set of size $k-1$.
...
\maciek{Suggestion: "Note that \OPT cannot have both..." (till the end of paragraph) You told me this in one sentence during the call. Maybe use the same formulation? I feel like one sentence can be enough to explain this \mahmoud{I removed one redundant sentence. Since this is the core of the proof let's keep it wordy.}}

\maciek{OPT's cost formula: make it one line by removing the expression with minimums. Move minimums to the paragraph text, by immediately expanding minimums (two steps in paragraph, instead of in the formula) \mahmoud{it is useful for reader's intuition to have the min till the end. So that they can remember why choice of $\lambda$ is not trivial.}}
