...
%Later, \RM collocates those pairs with two swaps.
Hence,
...
%We denote the cost w.r.t~$r_t$ by
$\mathit{OPT} (R_t) \geq 
...
i.e.~the first time \RM collocates $\{u,v\}$,
and not at any consequent time that \RM collocates $\{u,v\}$.
That is,
we charge \OPT a cost incurred by requests in
$\sigma_{\{u,w\}}$ or $\sigma_{\{u,v\}}$,
...
\begin{proof}[Proof of Theorem \ref{thm:k=2}]
	...
	First,
	we bound the costs incurred by \RM for requests that
	...
	the overall cost of requests in $\sigma_{\set{u,v}}$ incurred by Alg,
	...
	$\mathit{OPT} (\sigma_{\{u,v\}}(0,t]) \geq  \min{\{ \lambda,\alpha \}}$.
	%
	Otherwise,
	...
	consider times $t'$ and $t''$ as define previously,
	 and let 
	$R_t := \sigma_{\{u,w\}}(t'',t'] \cup \sigma_{\{u,v\}}(t',t] $.
	We define $R_{t'}$ for the collocation at $t'$ in an analogous way.
	Then,
	$\mathit{OPT} (R_t) = \mathit{OPT} (\sigma_{\{u,w\}}) 
	+ \mathit{OPT} (\sigma_{\{u,v\}}) $.
...
%	We consider cases (ii) and (iii) also for the pair $\{u,w\}$, symmetrically.
	...
	then
	$\mathit{OPT} (R_t) \geq
		\min{\{ \OPT_1(R_t), \OPT_2(R_t), \OPT_3(R_t) \}}  =
		\min{ \{ \lambda, \alpha \}}$.
	Otherwise,
	...
	We define times $t'' < t' < m$ similarly to the case where \RM collocates $\{u,v\}$.
