...
In any configuration, there exists an alone node that accompanies it.
We only issue requests between the current alone node from $S$, and some node that was initially collocated with it.
Repeating such requests, almost all node must visit the cluster $S_1$.
