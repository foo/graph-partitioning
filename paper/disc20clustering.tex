...
\begin{proof}
	...
	We distinguish a cluster $S$, where we form a ground set of size $k-1$.
	...
	Precisely, let $x_0$  be the alone node from $S_1$ and let $y_0$ be an arbitrary node that does not reside in $S$.
	...
	$S_1$ already contains a ground set of size $k-1$ (that it cannot split), hence \ALG must collocate $\{x_0, y_0\}$ in another cluster.
	

	We continue to construct ground sets containing the current alone node from $S$.
	...
	Then, we merge the ground sets of $x_1$ and the largest ground set $C$ s.t. and $C \neq \{x_0,y_0\}$~$I(C) = I(x_1)$.
	Continuing this, we observe a sequence $X := x_0, x_1, x_2\dots$ of alone nodes that visit $S$.
	...
	We claim that this procedure (of constructing ground sets) can continue as long as a perfect partition of ground sets still exists.
	...
	\ALG{} must collocate $\{x_i,y_i\}$ in a~cluster other than $S$.
	To this end, it replaces $x_i$ with another alone node $x_{i+1}$.
	To continue, we must guarantee that
	\maciek{elaborate}
	

	We end this procedure prematurely (before all nodes visit $S$) to keep \OPT's cost low.
	...
	We lower bound the length of the sequence $X := x_0, x_1,\dots$.
	\maciek{TODO}
	We exclude at most $(k-1) + ( \ell+1)$ nodes out of $k \cdot \ell$ nodes,
	therefore $|X| \geq k \cdot \ell - k - \ell$ \maciek{elaborate on exclude. Just sum up the real cost.}.
	...
	The competitive ratio is then: $\ALG/\OPT \geq (k \cdot \ell - k - \ell) / 2\in \Omega(k\cdot\ell)$.
\end{proof}
