...
We maintain all nodes of a component always collocated in a same cluster~\cite{repartition-disc}.
...
\mahmoud{we also use "singletone component". One is redundant.}
...
\maciek{The remainder of the paper used "configuration C" to denote the partition P. Easiest to adjust here. \mahmoud{are "configuration" and "partition" the same thing everywhere? Then let's use "partition" only.}}
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\IF{$C_1 \neq C_2$}
		\STATE {Unite $C_1$ and $C_2$ into one component $C'$ and
...
%		\COMMENT{i.e.~if not in the same cluster.}}       
		\IF{$C_1$ and $C_2$ are not collcoated}
		\STATE {Move to a partition closest to $P_I$, respecting all components in $\mathcal{C}$.} 
...
\PPL moves to a partition that minimizes the distance to the initial partition $P_I$.
As a result,
...
We emphasis that a repartitioning by \PPL replaces the current partition $P$ with a~perfect partition closest to $P_I$.
Meaning, it never moves to a partition beyond distance $\Delta^*$.

\begin{property} \label{prop:dist<OPT}
	Let $P$ be any configuration chosen by \PPL at any time.
	Then, $\Delta(P,P_I) \leq \Delta^*$.	
\end{property}
...
\begin{proof}
	...
	the distance before the repartitioning is $\Delta(P_{t-1},P_I) \geq | M^-|$.
	Analogously,
	the distance afterwards is $\Delta(P_{t},P_I) \geq | M^+|$.
	Thus,
	$|M| \leq \Delta(P_{t-1},P_I) + \Delta(P_{t},P_I)$.
	By Property \ref{prop:dist<OPT},
	$\Delta(P_{t-1},P_I) , \Delta(P_{t},P_I) \leq \Delta^* \leq \OPT$
	and thereby we have	
	$|M| \leq 2\cdot\OPT$.
	\maciek{the comma between $\Delta$ is confusing. BOTH are bounded? If so, expand the sentence and write both individual bounds.}
\end{proof}
...
\begin{proof}
	...
	That is, 
	once it reaches a configuration $P$ at distance $\Delta^* = \Delta(P, P_I)$,
...
%%
\maciek{"a same" I think it should always be "the same", does not sound right with a}
