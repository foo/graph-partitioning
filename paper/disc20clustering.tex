...
As in Henzinger et al. \cite{sigmetrics19_partitioning}, we assume that the communication graph can be perfectly partitioned in the clusters (no external requests are required).
For the learning model, we will assume that the migration cost is $\alpha=1$.
...
\maciek{Integer partition in non-augmented problem. Also: what to evict, rent or buy etc.}
...
In this section,
we present \RM,
 an algorithm for the \OBRP restricted to clusters of capacity 2,
i.e.~$k=2$.
