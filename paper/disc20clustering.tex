...
it has both pairs on the cut of  the same two clusters  (see Figure \ref{fig:TBD}).
...
%	we set $ t'' = -\infty, t'=0$.
	...
	If $t$ is the first time Alg collocates $\{u,v\}$ then we can charge
...
%	I.e., $ C^{t'}_{\set{u,v'}} = \lambda$.
	...
	incurred by Alg is
	$\lambda + 2\alpha$.
	We charge OPT the cost of serving requests in
	$\sigma_{\{u,v'\}}[t'',t']$ ($\sigma_{\{u,v\}}(t',t]$ ) 
	that it pays only within the interval $(t'', t']$ ($(t', t]$).
	Observe that for requests in $r_t := \sigma_{\{u,v'\}}(t'',t'] \cup \sigma_{\{u,v\}}(t',t]$,
	OPT must pay at least
	...
	\begin{enumerate}[label=\roman*.]
		\item
		OPT collocates $\{u,v'\}$ and $\{u,v\}$  at times  $t''$ and $t'$ respectively
...
$\mathit{Cost}_{\mathit{OPT}} (r_t) = \alpha$.
	...
	e.g.~by collocating a pair after partially paying for requests to this pair.
	 However,
	 any deviation only increases the cost to OPT.
	 \mahmoud{It would be better to describe and bound the "deviation" formally.}
	Therefore in any case,
 	$\mathit{Cost}_{\mathit{OPT}} (r_t) \geq  \min{ \{ \lambda, \alpha \}}$.
	
	It remains to charge both Alg and OPT for requests that do not lead to collocations until the  end of the sequence at $m$.
	Assume $\{u,v\}$ is not collocated at $m$
	and $0 < C^{m}_{ \{u,v\} } < \lambda $,
	which means Alg pays $C^{m}_{ \{u,v\} }$
	for requests to $\{u,v\}$ not accounted previously.
	If $\{u,v\}$ is not collocated in the initial configuration
	and Alg never collocates it through the sequence,
	then $ C^{m}_{ \{u,v\} } =| \sigma_{\{u,v\}}[0,m] |$.
	We charge OPT the cost
	$\mathit{Cost}_{\mathit{OPT}} (\sigma_{\{u,v\}}[0,t]) 
	\geq \min{ \{ \alpha, C^{m}_{ \{u,v\} } \} }$,
	for collocating this pair or (and) paying (partially) for requests in $\sigma_{\{u,v\}}[0,t]$.
	Else,
	...
	Let $r_{\{u,v\}} := \sigma_{\{u,v'\}} (t'',t'] \cup \sigma_{\{u,v\}} (t',t]$.
	Then,
	we apply cases (i) to (iii) for $t=m$ in an analogous way,
	after replacing $\lambda$ with $C^{m}_{ \{u,v\}}$.
	Thus, 
	we obtain
	 ...
	 This means,
	 the cost of r.h.s.~to OPT is counted at most two times in the expression
	 $\mathit{Cost}_{\mathit{OPT}} (r_{t'}) + \mathit{Cost}_{\mathit{OPT}} (r_t)$.
	 Hence,
	 for all collocations performed by Alg and for $t=m$,
	 OPT pays at least 
	 $\sum_{ 0 < t \leq |\sigma|} \mathit{Cost}_{\mathit{OPT}} (r_t) / 2$.
	Let
...
\begin{align*}
	\mathit{Cost}_{\mathit{ALG}} (\sigma)	/
	 ...
	 \Big)	 \big /
	 \Big(
	 \sum_{ t \in T} \mathit{Cost}_{\mathit{OPT}} (r_t) / 2
	 + \sum_{\{u,v\}}\mathit{Cost}_{\mathit{OPT}} (r_{\{u,v\}})  / 2 
	 \Big)	\\
	 &\leq
	 \Big(
	 \sum_{ t \in T}(\lambda + 2\alpha) +
	 ...
	 \Big)	 \big /
	 \Big(
	 \sum_{ t \in T} \min{ \{ \lambda, \alpha \}}  / 2
	 + \sum_{\{u,v\}} \min{ \{C^{m}_{\{u,v\}} , \alpha \}  / 2 } 
	 \Big)	\\
	 &=
	 \Big(
	\sum_{ t \in T} 3\alpha +
