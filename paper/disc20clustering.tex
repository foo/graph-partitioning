...
%	denoted by $x_1$.
	...
	we terminate the process.
	 ...
	 Hence, there are at most $\ell$ isolated nodes
	 at the point of termination.
	
	
	The number of isolated nodes decrease after each step.
	Consider the last step, $i^*$, when exactly $\ell+1$ isolated nodes exist before  $x_{i^*}$ joins to another ground set.
	I.e., there are less than $\ell+1$ isolated nodes after the step $i^*$.
	Then, at the end of step $i^*-1$,
...
%
    ...
    $\sigma := \{ \{x_0,y_0\}, \dots, \{x_{i^*-1},y_{i^*-1}\}  \}$.
...
%	there is no request to $x^*$ or $y^*$ in $\sigma$.
...
%	some ground set grows by one node at each of these steps.
	...
	Let $\mathcal{C}^*$ be the set of all ground sets after serving the last request in $\sigma$.
	Thus, $\mathcal{C}^*$ includes ground sets $B$,
	$\{x_0,y_0\}$,
	and $\ell+1$ singleton ground sets.
%	(including $x^*$ and possibly $y^*$).	
	Each time we join the special node to the largest ground set of same origin.
	...
	Therefore,  $|\mathcal{C}^*| \leq 1 + 1 + (\ell+1)  + (\ell-1) = 2\ell+2$.
