...
A straightforward analysis of the algorithm from Section~\ref{sec:k3} results in the bound $O(\ell^2)$, and the analysis must be improved in two places. One must bound the cost of each of $O(\ell)$ reconfigurations in~phase by a~constant, and show that the optimum offline algorithm must pay for a~significant portion of inter-cluster requests \maciek{how to concisely say that there is also a~migrate possibilty}.
