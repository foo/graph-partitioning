...
%	then $I(C)$ is undefined.
	...
	the adversary issues external requests between them until \ALG collocates them.
	...
	We continue joining the current special node to a~larger ground set.
	Precisely, for the isolated node $x_i$ currently collocated with $B$,
	 we issue a request between $x_i$ and a node $y_i$
	 in the largest ground set $C$
	 s.t.~$I(C) = I(x_i), C \neq \{x_0,y_0\}$.

\mahmoud{
	If a feasible partition exists 
	that collocates the new ground set $\{x_i,\} \cup C $,  
	then $x_i$ is replaced by some isolated node
	 $x_{i+1} \not\in \{y_0, x_0, \dots, x_i\}$.
%	 similarly to $x_0$, and we continue creating such ground sets.
%	We describe the termination condition shortly.
	Otherwise,
	the current partition is not feasible,
	 we terminate the process and we let $x^* := x_i, y^*:=y_i$. 
	 At this point,
	 ...
	 Then there must be two isolated nodes $u$ and $v$ with the same cluster of origin,
	 i.e., $I(\{u\}) = I(\{v\})$.
	 ...
	 Therefore $P$  has all ground sets constructed so far collocated, i.e.,
	 it is a feasible partition, which contradicts our assumption.
}
