...
\maciek{Title discussion: "Online Balanced Graph Partitioning: Understanding the role of clusters". Reason: all our results most notably pertain to the number of clusters.}
...
\mahmoud{"Before serving a request": does this mean that only after a request arrives and before serving it we migrate? In our algorithms we migrate after serving $\lambda$ requests. Do we have discrepancy with the model?}
...
%	else,
%	we set $ t'' = -\infty, t'=0$.
	We denote the set of all requests to $\set{x,y}$
...
%	Clearly,	OPT cannot have both pairs $\{u,v\}$ and $\{u,v'\}$
	collocated at the same time.

	Next,
	we focus on the cost of requests to $\sigma_{\set{u,v}}[t',t]$ incurred by OPT.
	If $t$ is the first time Alg collocates $\{u,v\}$ then
	OPT must pay $\lambda$ for requests $\sigma_{\set{u,v}}[t',t]$ or
	$\alpha$ for collocating the pair.
	Thus, it pays $\min{\{ \lambda,\alpha \}}$.
	Else,
...
%	I.e., $ C^{t'}_{\set{u,v'}} = \lambda$.
	If $t'$ is not the first time that Alg has $\{u,v'\}$ collocated
	then let $t''<t'$ be the last time before $t'$ Alg separates $\set{u,v'}$.
	Else,
	$\{u,v'\}$ is not collocated in the initial configuration and let $t''=0$.
	We distinguish two cases for pairs $\{u,v\}$ and $\{u,v'\}$
	in OPT's choice of configuration,
	w.r.t.~requests $r_t := \sigma_{\{u,v'\}}[t'',t'] \cup \sigma_{\{u,v\}}[t',t]$.
	\begin{enumerate}[label=\roman*.]
		\item OPT has one of the pairs,
		e.g.~$\{u,v'\}$,
		already collocated (paying zero for $\sigma_{\{u,v'\}}[t'',t']$)
		and pays for requests to the other one,
		i.e., it pays $\lambda$ for $\sigma_{\{u,v\}}[t',t]$.
		\item OPT collocates $\{u,v\}$  and $\{u,v'\}$ (separately) and
		pays only $2\alpha$ for all requests in $r_t$.
		\item 
	\end{enumerate}
	 In either case,
	 OPT pays $\min{ \{\lambda, 2\alpha \}}$.
	OPT may deviate from these cases,
	e.g.~by collocating a pair after partially paying for requests to this pair.
	 However, any deviation only increases the cost to OPT.
	Therefore in any case,
	OPT pays at least $\min{ \{\lambda, 2\alpha \}}$ for requests in
	$\sigma_{\{u,v'\}}[t'',t'] \cup \sigma_{\{u,v\}}[t',t]$,
	which gives the cost $\min{ \{\lambda / 2, \alpha \}}$ per pair.
	%	which gives the competitive ratio 
	%	$(\lambda + 2\alpha)/ \alpha = 4$.
	
	It remains to charge both Alg and OPT for requests that do not lead to collocations when the sequence ends at $t$.
	Assume $\{u,v\}$ is not collocated at $t$
	and $C^{t}_{ \{u,v\} } > 0$,
	which is the cost of $ \{u,v\}$ to Alg.
	If Alg never has $\{u,v\}$ collocated through the sequence then OPT pays
	$\min{ \{C^{t}_{ \{u,v\} } , \alpha \} }$ for  requests to $\{u,v\}$
	or for collocating it at cost $\alpha$.
	Else,
	there exists an intersecting pair $\set{u,v'}$ and
	times $t'' < t' < t$ as defined previously.
...
Missing:
\maciek{Missing the case of the first-time collocation. What if t' does not exist? What if t'' does not exist? 
\mahmoud{added remark for this case: $t''=0$
\maciek{But t' might not exist (as it might be the first collocation).}}}

Minor:
\maciek{"It can be easily verified"... True, but avoid such formulation. Something along these lines: if opt migrates earlier than t', then additionally pays for X, if later than Y.}

Tiny:
<<<<<<< Updated upstream
\maciek{$\sigma_t$ undefined (I know what it is, though). \mahmoud{added inline def. Do we have a global def for this?}
\maciek{Not yet, but we still must write the "Problem definition" section (yes, we don't have one!), and we can put it there.}}


\maciek{Define $C^t$ to use once \mahmoud{already defined! I don't get it.
\maciek{discussed}}}
=======
\maciek{$\sigma_t$ undefined (I know what it is, though). \mahmoud{added inline def. Do we have a global def for this?}}
\maciek{Define $C^t$ to use once \mahmoud{already defined! I don't get it.}}
\maciek{A word "Obviously" is unnecessary, and most people, including me are allergic to it in writing
\mahmoud{obviously that's true! connect the two sentences}}
>>>>>>> Stashed changes
