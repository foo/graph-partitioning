...
\begin{figure}[H]
	...
	An arrow, e.g.~from $b_1$ to $a_1$ means  that $b_1$ replaces $a_1$.\maciek{Maybe: "An arrow from $a$ to $b$ means that $b$ replaces $a$.". Or "$b$ migrates in place of $a$."} }
	\label{fig:cyclic}
\end{figure}
...
However,
we can charge the whole cost \maciek{Whole cost unclear, I think} to \OPT only the first time \RM collocates a~pair,
...
$\sigma:=\{\sigma_1,\dots, \sigma_m\}$.
	...
	We may use $\sigma_{\set{x,y}}$ whenever
	 the interval $[t_1,t_2]$ is clear from the context \maciek{As a reader I would be horrified seeing this sentence, expecting shenanigans. Better to fix it in the paragraph}.
	%
...
%	However, it can have one of the pairs already collocated prior to its respective interval.
	...
	or (resp., and) it pays (resp., up to) $\lambda$ for serving (resp., some of) requests in $\sigma_{\{u,w\}}$ \maciek{Rewrite without these resp.s. Also, see the comment on "or (and)" below}. 
	...
	separating $\{u,v\}$.
	We define times $t'' < t' < m$ similarly to the former case \maciek{doing thing "similarly" is a trap zone in proofs, and we should avoid even suggesting it. I think that here you mean defining exactly the same.}.
	Let $R^*_{\{u,v\}} := \sigma_{\{u,w\}} (t'',t'] \cup \sigma_{\{u,v\}} (t',m]$.
	Then,
	\OPT must pay for collocating at least one pair or (and) \maciek{"or (and)" usually should be "and possibly". Does this make sense here?} serving requests 
...
%	(\mahmoud{See figure \ref{fig:doubleCount}}). 
	...
	$\OPT (R_{t'}) + \OPT (R_t)$
	and  
	$\OPT (R_{t'}) + \OPT(R^*_{\{u,v\}})$ \maciek{Propose how to formalize (but up to you). I think this is classic bound of maximum by an average argument. We actually use it in this paper already in \TAlg, line 407 at the time of writing.}.
	Hence,
