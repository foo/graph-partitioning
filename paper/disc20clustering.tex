...
\begin{abstract}
	...
	\maciek{How to put here the information that it is for deterministic algorithms only? This sentence is already very long.}lower bound of $\Omega(k\cdot \ell)$ on the
...
A straightforward analysis of the algorithm from Section~\ref{sec:k3} results in the bound $O(\ell^2)$, and the analysis must be improved in two places. One must bound the cost of each of $O(\ell)$ reconfigurations in~phase by a~constant, and show that the optimum offline algorithm must pay significant cost for inter-cluster requests.
...
A~\emph{ground set} is a~subset of nodes such that if an algorithm does not have them all collocated,
then the split parts communicate indefinitely until the algorithm collocates them.
%A~\emph{ground set} is a~group of nodes that  communicate indefinitely when an algorithm splits them (places at different clusters).
This ensure us that any competitive algorithm would maintain a~partition of ground sets into clusters.
As a result,
 a ground set is revealed to an algorithm only after the algorithm splits it.
 More precisely,
the adversary constructs ground sets depending on the choices of a~given deterministic online algorithm.
%Once we construct a~ground set, it lasts until the algorithm terminates.
We say that a~ground set is \emph{singleton} if it contains only one node,
and we refer to such node as an \emph{isolated} node.


We start by constructing a~ground set of size $k-1$ on an arbitrarily chosen cluster.
In any partition, there must exist an isolated node collocated with the ground set of size $k-1$, and we call it \emph{special}.
We issue requests between the current special node, and some node that was initially collocated with it.
By repeating such requests, almost every node is collocated with the first ground set at some point .
...
%	then $I(C)$ is undefined.
	...
	Initially, all nodes are isolated (i.e., in singleton ground sets).
	...
	Each cluster hosts exactly $k$ nodes, and in any partition, a~single isolated node must be collocated with $B$.
	We say that this isolated node that accompanies the ground set $B$ is \emph{special}.
	We denote the first special node by $x_0$.

	Then, we join the special node to a~larger ground set to force its eviction.
	Precisely,
	we create a~ground set $\{x_0, y_0\}$, 
	where $x_0$ is the current special node and $y_0$ is an arbitrary isolated node.
	Since \ALG does not have $\{x_0,y_0\}$ collocated, 
	we issue an external request between them so that \ALG collocates them.
	...
	\ALG must replace $x_0$ with another isolated node.
%	denoted by $x_1$.

	We proceed by joining the current special node to a~larger ground set,
	as long as a feasible partition exists afterwards.
	Consider the isolated node $x_i$ currently collocated with $B$.
	 We issue a request between $x_i$ and some node $y_i \in C_i$,
	 where $C_i$ is the largest ground set
	 s.t.~$I(C_i) = I(x_i), C_i \neq \{x_0,y_0\}$,
	 only if  a feasible partition exists
	 that collocates all nodes in $\{x_i\} \cup C_i $.
	Any such feasible partition replaces $x_i$ with some isolated node $x_{i+1}$,
	as the new ground set $\{x_i\} \cup C_i $  may not be ever split.
	Otherwise,
	if no such feasible partition exists,
	 then we terminate the process and
	 we let $x^* := x_i, y^*:=y_i$. 
	 
	 Once the process terminates,
	 there are at most $\ell$ isolated nodes left 
	 that are not contained in $\{x^*,y^*\}$.
	 Assume for contradiction that this is not the case,
	 i.e., there are at least $\ell+1$ isolated nodes left
	 in addition to $x^*$ and possibly $y^*$.
	 ...
	 Hence, at the point of termination,
	 there are at most $\ell$ isolated nodes not in $\{x^*,y^*\}$,
	 or at most $\ell+2$ isolated nodes overall.
...
\begin{comment}
	...
	We show that if at least $\ell$ isolated nodes exist (including $x_i$), then there exists a~perfect partition of ground sets after joining $x_i$ to a~ground.
	To show this claim, we consider a~partition, where the largest ground set of each cluster originates from it (excluding the ground set $\{x_0, y_0\}$). \mahmoud{this sentence doesn't make sense.}
	...
	\mahmoud{why  $\{x_0, y_0\}$ must be present there?}
	...
	\mahmoud{but didn't you assume there is no isolated on $I(x_i)$?}
	...
	\mahmoud{here you are making an exception to the already established process. Instead, adapt the termination condition so that it triggers prematurely, by saying something like "issue the last request only if there will be a feasible partition." }
...
\end{comment}
	...
	we never issue requests between nodes $x^*, y^*$
	 which are separated in this partition.
...
%	some ground set grows by one node at each of these steps.
	Consider any ground set $C^*$ at the point of termination.
	This ground set has grown exactly $|C^*|-1$ times until the termination. 
	Let $\mathcal{C}^*$ be the set of all ground sets at the point of termination.
	Thus, $\mathcal{C}^*$ includes ground sets $B$,
	$\{x_0,y_0\}$,
	and up to $\ell+2$ singleton ground sets.
%	(including $x^*$ and possibly $y^*$).	
	Each time we join the special node to the a largest ground set.
	Hence, every  other ground set in $\mathcal{C}^*$ has grown maximally.
	There are at most $\ell-1$ such ground sets in addition to the ones 
	we considered so far,
	one per cluster except the cluster containing $B$. 
	Therefore,  $|\mathcal{C}^*| \leq 1 + 1 + (\ell+2)  + (\ell-1) = 2\ell+3$.
	Note that among all non-singleton ground sets in $\mathcal{C}^*$,
	 only $B$ does not grow during the process.
	 Thus,
    the total number of times that 
	a ground set in $\mathcal{C}^*$  has grown is
	\begin{align*}
	\sum_{C^* \in \mathcal{C}^* } (|C^*|-1) - (k-1) &= 
	\sum_{C^* \in \mathcal{C}^* } |C^*| - 
		\sum_{C^* \in \mathcal{C}^* } 1 - (k-1) \\
	&\geq  k  \ell - (2\ell+3) - (k-1) =  (k-2)(\ell-1) - 4,
	\end{align*}
	which is the least number of swaps performed by \ALG.
